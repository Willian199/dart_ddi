...
* **Factories**: All BaseFactory factory methods now require `ddiInstance: DDI` parameter:
* **InstanceFactoryMixin**: Changed `currentFactory` parameter to `ddiInstance`:
  * `createInstance()`: Replaced `DDIBaseFactory? currentFactory` with `required DDI ddiInstance`.
  * `createInstanceAsync()`: Replaced `DDIBaseFactory? currentFactory` with `required DDI ddiInstance`.
* I made some changes to how the DDIModule handles futures. Now it's necessary to use `await` to retrieve errors from the registration process.
